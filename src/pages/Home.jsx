  ...
  const fetchPosts = async () => {
    ...
    if (!error && data) {
      const postsWithCounts = await Promise.all(
        data.map(async (post) => {
          const { count } = await supabase.from("comments").select("id", { count: "exact", head: true }).eq("post_id", post.id).eq("is_deleted", false);
          return {
            ...post,
        ...
        {loading ? (
          ...
          <section className={styles.emptyState}>
            <div className={styles.hero}>
              ...
              <p className={styles.heroSub}>첫 번째 작품을 올리면 이 공간이 작가 노트와 이미지 중심의 포트폴리오 홈으로 완성됩니다.</p>
            </div>
          ...
          <>
            ...
            <section className={styles.featuredSection}>
              <div className={styles.sectionHeader}>
                ...
                <p className={styles.sectionDescription}>이미지 한 장과 짧은 기록이 모여 페이지의 분위기를 만듭니다.</p>
              </div>
            ...
            {recentPosts.length > 0 && (
              <section className={styles.recentSection}>
                <div className={styles.sectionHeader}>
                  ...
                  <p className={styles.sectionDescription}>최신 게시물부터 흐름이 자연스럽게 이어지도록 카드형 목록으로 정리했습니다.</p>
                </div>
